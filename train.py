  ...
  for epoch in range(epoch_str, hps.train.epochs+1):
    ...
    for batch_idx, (x, _) in enumerate(train_loader):
      ...
      with autocast(enabled=hps.train.fp16_run):
        enc_z_gt,z_norm,y,flow1_pyramid_2,x_enc,x,mu,log_var,flow2_pyramid_1 = net_g(x.detach())
        flow1_pyramid_2.reverse()
      ...
      with autocast(enabled=hps.train.fp16_run):
        x_enc_reverse,enc_z,z_norm,flow1_pyramid_1,flow2_pyramid_2 = net_g(x_enc.detach(),reverse=True)
        # enc_z,z_norm_r,flow2_pyramid_2 = net_g(x_enc.detach(),reverse=True)
        with autocast(enabled=False):
          flow2_pyramid_2.reverse()
      ...
      with autocast(enabled=hps.train.fp16_run):
        enc_z = enc_z.clone()
        enc_z_gt = enc_z_gt.clone()
        y_dec,y_pyramid,y_dec_post = net_d(enc_z,flow2_pyramid_2)
        y_dec_gt,y_pyramid_gt,y_dec_gt_post = net_d(enc_z_gt,flow2_pyramid_2)
        with autocast(enabled=False):
          for i,yy in enumerate(y_pyramid):
            loss_pyramid=loss_pyramid + torch.abs(yy-flow1_pyramid_2[i].detach()).mean()
          for i,yy in enumerate(y_pyramid_gt):
            loss_pyramid_gt=loss_pyramid_gt + torch.abs(yy-flow1_pyramid_2[i].detach()).mean()
          loss_y_dec_recon = F.mse_loss(y_dec,x_enc)
          loss_y_dec_recon_gt =  F.mse_loss(y_dec_gt,x_enc)
          loss_y_dec_post_recon = F.mse_loss(y_dec_post,x_enc)
          loss_y_dec_recon_gt_post =  F.mse_loss(y_dec_gt_post,x_enc)
          loss_d_total = loss_pyramid+loss_y_dec_recon+loss_pyramid_gt+loss_y_dec_recon_gt+loss_y_dec_recon_gt_post+loss_y_dec_post_recon
      optim_d.zero_grad()
      ...
      if global_step%hps.train.log_interval == 0:
        ...
        scalar_dict.update({'loss/d/loss_y_dec_recon':loss_y_dec_recon,'loss/d/loss_y_dec_recon_gt':loss_y_dec_recon_gt,"loss_y_dec_post_recon":loss_y_dec_post_recon,
        "loss_y_dec_recon_gt_post":loss_y_dec_recon_gt_post,"loss_pyramid":loss_pyramid,'loss_pyramid_gt':loss_pyramid_gt,'loss/d/total':loss_d_total})
        ...
        image_dict = { 
            ...
            "img/flow1_pyramid" : utils.plot_images_to_numpy(flow1_pyramid_2),
            "img/flow2_pyramid_1" : utils.plot_images_to_numpy(flow2_pyramid_1),
            "img/flow3_pyramid_1_1" : utils.plot_images_to_numpy(y_pyramid),
